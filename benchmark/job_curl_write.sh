...
for i in $(seq 2 $4); do
  if [ $UPLOAD_FILE_DIRECTLY ]; then
    curl -X POST --data-binary "@$3" -H "Authorization: Bearer $ACCESS_TOKEN" $URL > /dev/null
  else
    cat "$3" | curl -X POST --data-binary @- -H "Authorization: Bearer $ACCESS_TOKEN" $URL > /dev/null
  fi
done
